...
class RequestApi(object):
    ...
    def all_api_request(self):
        ...
        while True:
            ...
            time.sleep(1)
        # 5 . 获取结果
        self.yyzwzresult=self.get_result_request(taskid=taskid)
...
def identify(file_path):
    api = RequestApi(appid="6e3c02b8", secret_key="d39fdd7602ae8696393895c03b1d7a39", upload_file_path=file_path)  
    api.all_api_request()
    jsres=api.yyzwzresult
    text=jsres['data']
    jsonres=json.loads(text)
    yuyinstr=''
    for i in range(len(jsonres)):
        yuyinstr+=jsonres[i]['onebest']
        finstr=yuyinstr.replace("。","")    
    return yuyinstr
